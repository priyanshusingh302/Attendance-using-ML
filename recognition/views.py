...
def update_percent(request,pcents):
    messages.success(request, f'{pcents}% Done')
...
def train(request):
    ...
    cnt = 0
    for people in os.listdir(training_dir):
        curr_directory = os.path.join(training_dir, people)
        cnt += len(os.listdir(curr_directory))
